...
const { ResolveExpressions } = require('../../custom-steps');
const prettyTime = require('pretty-ms');
const {WebBuilder} = require('../web/autokin-web');

const takeScreenshot = (name, before = true) => {
    if (!WebBuilder.page) return Promise.resolve();

    name = name.replace(/[^a-z0-9]/gi, '_').toLowerCase();

    return Promise.resolve(WebBuilder.screenshot('./'+name + (before ? '-before' : '-after')));
}
...
class AutokinFormatter extends JsonFormatter {
    constructor(options) {
        ...
        options.eventBroadcaster.on('test-step-started', ({ testCase, index }) => {
            ...
            if(pickleStep) {
                takeScreenshot(pickleStep.text);
                if (process.send) {
                    try {
                    process.send({
                        type: 'test-step-started',
                        data : {
                            step : gherkinKeyword + pickleStep.text,
                            expressions : ResolveExpressions(pickleStep.text)
                        }
                    });
        ...
        options.eventBroadcaster.on('test-step-finished', ({ testCase, index, result }) => {
            ...
            if (pickleStep) {
                if (process.send) {
                    ...
                    takeScreenshot(pickleStep.text, false);
                    } catch(e) {
                        console.log(e);
                    }
